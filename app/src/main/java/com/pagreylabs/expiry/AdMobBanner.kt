...
/** Free-tier banner. It remains inert until real AdMob IDs are configured. */
@Composable
fun ExpiryAdBanner(modifier: Modifier = Modifier) {
    val context = LocalContext.current
    val activity = context as? Activity ?: return
    val premium = remember {
        context.getSharedPreferences("expiry_settings", 0)
            .getBoolean("premium_enabled", false)
    }
    if (premium || BuildConfig.ADMOB_BANNER_AD_UNIT_ID.contains("NOT_CONFIGURED")) return
